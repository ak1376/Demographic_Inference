...
# ):
#     """
#     Optimize params using NLopt's BOBYQA algorithm.
#     """
...
#         data,
#         model_func,
#         None,  # bounds are handled separately in NLopt
#         None,
#         verbose,
#         multinom,
#         flush_delay,
...
#     # Project fixed parameters and log-transform the initial parameters
#     p0 = np.log(_project_params_down(p0, fixed_params))

#     # Prepare bounds in log-space
#     if lower_bound is not None:
#         lower_bound = np.log(_project_params_down(lower_bound, fixed_params))
#     if upper_bound is not None:
#         upper_bound = np.log(_project_params_down(upper_bound, fixed_params))

#     # Set up NLopt optimizer (BOBYQA)
...
#     def objective_function(log_params, grad):
#         """
#         Wrapper around _object_func to be used by NLopt.
#         It expects log-transformed parameters and returns the negative log-likelihood.
#         """
#         if grad.size > 0:
#             raise NotImplementedError("BOBYQA does not use gradient information.")

#         params = np.exp(log_params)  # Convert back from log-space
#         return _object_func(params, *args)
...
#     except nlopt.RoundoffLimited:
#         print(
#             "nlopt.RoundoffLimited occured, other jobs still running. Users might want to adjust their boundaries or starting parameters if this message occures many times."
#         )
#         opt_val = -np.inf
#         xopt = [np.nan] * len(p0)
