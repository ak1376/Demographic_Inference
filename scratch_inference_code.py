...
vcf_data = allel.read_vcf(
    "/sietch_colab/akapoor/Demographic_Inference/GHIST-bottleneck.vcf.gz"
)
...
dd = dadi.Misc.make_data_dict_vcf(
    "/sietch_colab/akapoor/Demographic_Inference/GHIST-bottleneck.vcf.gz",
    "/sietch_colab/akapoor/Demographic_Inference/wisent.txt",
)


fs = dadi.Spectrum.from_data_dict(
    dd, ["wisent"], projections=[2 * num_samples], polarized=True
)
...
with open(
    "/sietch_colab/akapoor/experiments/linear_model_bottleneck/dadi_dict.pkl", "rb"
) as file:
...
features = np.concatenate(
    (
        opt_params.reshape(1, opt_params.shape[0]),
        opt_params_moments.reshape(1, opt_params_moments.shape[0]),
    ),
    axis=1,
)
...
model.load_state_dict(
    torch.load(
        "/sietch_colab/akapoor/experiments/linear_model_bottleneck/final_model.pth"
    )
)
...
with open(
    "/sietch_colab/akapoor/experiments/linear_model_bottleneck/linear_regression_model.pkl",
    "rb",
) as file:
